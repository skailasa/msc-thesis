...
leaf box, this leads to $O(\kappa N)$ complexity for direct calculations at the
...
In software engineering terms this is inconvenient as it leads to the requirement to implement
problem-specific codes for each kernel one may encounter. This leads to a
productivity overhead in either designing a single, extensible
...
domain, one may be needlessly refining the boxes in some regions, and calculating expansion
coefficients for empty boxes. Therefore, although the asymptotic complexity of
...
parallelism in practical implementations. Specifically, the computations
...
\textbf{\gls{task-level-parallelism}}. In addition to calculating the operators
using multiprocessing, there is scope for implementing
\textbf{\gls{data-level-parallelism}} to find these coefficients. For example,
