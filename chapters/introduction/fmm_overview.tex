...
for systems of moderate size. In realistic systems, one may be interested in
interactions involving $10^{6}$ to $10^8$ particles, making it impractical to
directly compute all interactions.

This chapter introduces the earlier, analytic, \gls{FMM}, with the Kernel-Independent
\gls{FMM} which is the main focus of this thesis presented later.
...
and developing upon these algorithms. The analytic
\gls{FMM} represented a sea change for $N$-Body simulation as it manages to
...
equipped with rigorous error bounds, making fast and accurate massive $N$-Body simulations
feasible on available computing hardware. It's success has been such that it is
regarded as one of the key developments in numerical algorithms in the twentieth century \cite{Cipra:2000:SN}.
...
for two points with spherical coords. $(r, \theta, \phi)$ and
$(\rho, \alpha, \beta)$ respectively, and $\gamma$ is the angle subtended between them.
Therefore, the multipole expansion goes to,
...
condition means that the estimate of the potential at a given evaluation point
calculated using the multipole expansion is only valid in the far-field, which is
often tuned empirically for different systems as it's user defined.
If instead the expansion is taken with centre at the evaluation point,
...
a multipole or local expansion at an analytic point is bound by $O(N)$, where
there are $N$ points taken into account by the expansion.
...
The region of convergence for ... is limited ... therefore we need to use a
tree structure.
