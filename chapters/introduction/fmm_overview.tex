...
Futrhermore, exact operations exist for shifting the center of these expansions,
...
\begin{enumerate}
    ...
    parent but are not adjacent to the current box itself. This is also known as
    the multipole-to-multipole operation, or \textbf{\gls{M2L}}. At the end of
    ...
    as well as in the \gls{near-neighbours}, which is computed directly.
\end{enumerate}
...
Beginning with upward pass, at leaf level each particle contributes to one expansion
so P2M of the order $Np^2$ - $p^2$ is the order of operations for the multipole
...
translation operators are what lead to the $\mathcal{O}(n)$ complexity.
