...
to machine precision, and \gls{PyExaFMM} uses a value of, $\text{tol} = 1/ (4 \cdot \text{EPS} \cdot \max (\alpha I + \Sigma^*\Sigma)^{-1} )$,
where EPS is machine precision, and $\max ( \alpha I + \Sigma^*\Sigma)^{-1}$ is the largest
entry in $(\alpha I + \Sigma^*\Sigma)^{-1}$. This has been determined empirically to provide
